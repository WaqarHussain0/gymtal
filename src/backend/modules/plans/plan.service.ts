...
export class PlanService {
  ...
  async createPlan(payload: CreatePlanDto) {
    const { name, duration, price, members, features } = payload;

    const existingPlan = await this.findByName(name);
    if (existingPlan) {
      throw new Error(`Plan with name ${name} already exists, please use a different name.`);
    }

    return await this.planEntity.create({ name: name.trim(), duration, price, members, features });
  }
  ...
  async updatePlan(id: string, payload: Partial<CreatePlanDto>) {
    ...
    if (name) {
      const existingPlan = await this.findByName(name as string);
      if (existingPlan && existingPlan._id.toString() !== id) {
        throw new Error(`Plan with name ${name} already exists, please use a different name.`);
      }
    }

    return await this.planEntity.findByIdAndUpdate(id, {
      name,
      duration,
      price,
      members,

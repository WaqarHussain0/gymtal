...
const MembershipPeriodTable: React.FC<IMembershipPeriodTableProps> = ({ membershipPeriods, userId }) => {

    const [showRenewMembershipDialog, setShowRenewMembershipDialog] = useState<boolean>(false);
    const handleCloseDialog = useCallback(() => setShowRenewMembershipDialog((prev) => !prev), [])
    return (
        <Card className="w-full">

            <CardHeader>
                <CardTitle className="flex items-center justify-between">Membership Periods
                    <Button size="sm" onClick={handleCloseDialog}>Renew Membership</Button>
                </CardTitle>
            </CardHeader>
